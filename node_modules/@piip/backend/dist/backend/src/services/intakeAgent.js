...
async function processClientMessage(sessionId, message) {
    ...
    if (missingFields.length === 0) {
        ...
        try {
            ...
            const collectedForClassify = session.collectedData;
            const classifyResult = await classifyCase({
                title: typeof collectedForClassify["title"] === "string"
                    ? collectedForClassify["title"]
                    : template.name,
...
async function createCaseFromSession(session, template) {
    ...
    const title = (typeof collected["title"] === "string"
        ? collected["title"]
