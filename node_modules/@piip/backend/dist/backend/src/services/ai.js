...
if (PROVIDER === "azure-openai") {
    try {
        ...
        logger_1.default.warn("Azure OpenAI SDK not available: %s", msg);
    }
}
    ...
    try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        logger_1.default.warn("OpenAI SDK not available: %s", msg);
    }
}
...
async function extractEvidenceContext(evidence) {
...
${context}`;
    try {
        ...
        logger_1.default.error("LLM parse error: %s", msg);
        return {
...
async function analyzeCase(evidences) {
...
${combined}`;
    try {
        ...
        const msg = e instanceof Error ? e.message : String(e);
        logger_1.default.error("LLM parse error (case): %s", msg);
        return {
