 ...
 * 증거 자료 AI 분석
 */
router.post("/analyze-evidence", auth_1.verifyJWT, async (req, res) => {
    try {
 ...
 * 사건 결과 예측
 */
router.post("/predict-outcome", auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        const prediction = {
            ...
            prediction: {
                ...
                estimatedDuration: Math.floor(durationDays * (0.8 + Math.random() * 0.4)),
 ...
 * 패턴 감지 및 분석
 */
router.post("/detect-patterns", auth_1.verifyJWT, async (req, res) => {
    try {
 ...
 * AI 기반 보고서 요약 생성
 */
router.post("/generate-report-summary", auth_1.verifyJWT, async (req, res) => {
    try {
