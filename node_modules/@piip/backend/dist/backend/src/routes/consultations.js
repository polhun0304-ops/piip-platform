...
router.post("/", auth_1.verifyJWT, (0, validate_1.validateBody)(createSchema), async (req, res) => {
    try {
        ...
        const { type, channel, timezone, scheduledAt, durationMinutes, detectiveId, caseId, legalAdviceDisclaimerAck, recordingConsent, privacyPolicyAck, } = (validatedBody ?? req.body);
        if (!req.user)
...
router.get("/", auth_1.verifyJWT, (0, validate_1.validateQuery)(listQuerySchema), async (req, res) => {
    try {
        ...
        const { status, detectiveId, caseId, clientUserId, startDate, endDate, page, pageSize, } = (validatedQuery ?? req.query);
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
        const repo = database_1.AppDataSource.getRepository(Consultation_1.Consultation);
        const where = {};
        ...
        if (startDate && endDate) {
            where.scheduledAt = (0, typeorm_1.Between)(new Date(String(startDate)), new Date(String(endDate)));
        }
        else if (startDate) {
            where.scheduledAt = (0, typeorm_1.MoreThanOrEqual)(new Date(String(startDate)));
        }
        else if (endDate) {
            where.scheduledAt = (0, typeorm_1.LessThanOrEqual)(new Date(String(endDate)));
        }
...
router.patch("/:id/status", auth_1.verifyJWT, (0, validate_1.validateBody)(statusSchema), async (req, res) => {
    try {
        ...
        if (status)
            c.status = status;
        if (cancelReason)
            c.cancelReason = String(cancelReason);
        if (summaryNote)
            c.summaryNote = String(summaryNote);
        const saved = await repo.save(c);
        return res.json(saved);
    }
    catch (e) {
