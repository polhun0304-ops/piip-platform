...
exports.classifyCase = classifyCase;
exports.calculateMatchScore = calculateMatchScore;
exports.autoAssignDetective = autoAssignDetective;
exports.getAvailableCasesForSelection = getAvailableCasesForSelection;
exports.manualAssignDetective = manualAssignDetective;
 ...
 * 사건 카테고리 자동 분류
 */
async function classifyCase(caseData) {
...
위 사건을 가장 적합한 카테고리로 분류하세요.`;
    ...
    for (const [category, words] of Object.entries(keywords)) {
        if (words.some((w) => caseData.title.includes(w) || caseData.description.includes(w))) {
 ...
 * AI 기반 탐정 매칭 점수 계산
 */
async function calculateMatchScore(caseData, detective, category) {
    let score = 0;
    const reasons = [];
    // 1. 전문 분야 매칭 (40점)
    ...
    if (specialty) {
        ...
        reasons.push(`${category} 분야 ${specialty.level} (${specialty.casesHandled}건 경험)`);
    }
    else {
    ...
    if (utilization < 0.5) {
        ...
        reasons.push(`여유 있음 (${detective.currentCaseCount}/${detective.maxConcurrentCases})`);
    }
    else if (utilization < 0.8) {
        score += 15;
        availabilityMatch = true;
        reasons.push(`보통 (${detective.currentCaseCount}/${detective.maxConcurrentCases})`);
    }
    else if (utilization < 1.0) {
        score += 10;
        reasons.push(`바쁨 (${detective.currentCaseCount}/${detective.maxConcurrentCases})`);
    }
    else {
        score += 0;
        reasons.push(`포화 (${detective.currentCaseCount}/${detective.maxConcurrentCases})`);
    }
    // 4. 성과 (20점)
    ...
    if (detective.successRate >= 90 && detective.averageRating >= 4.5) {
        ...
        reasons.push(`우수 성과 (성공률 ${detective.successRate}%, 평점 ${detective.averageRating})`);
    }
    else if (detective.successRate >= 75 && detective.averageRating >= 4.0) {
        score += 15;
        performanceMatch = true;
        reasons.push(`양호 성과 (성공률 ${detective.successRate}%, 평점 ${detective.averageRating})`);
    }
    else {
        score += 10;
        reasons.push(`보통 성과 (성공률 ${detective.successRate}%, 평점 ${detective.averageRating})`);
    }
    // 상태 체크
 ...
 * 최적 탐정 자동 배정
 */
async function autoAssignDetective(caseId, options = {}) {
    const { minScore = 50, maxCandidates = 5 } = options;
    ...
    // 각 탐정별 매칭 점수 계산
    const candidates = await Promise.all(detectives.map(async (detective) => {
        const { score, reason } = await calculateMatchScore(caseData, detective, category);
 ...
 * 탐정 선택 가능한 사건 목록 조회
 */
async function getAvailableCasesForSelection() {
    const caseRepo = database_1.AppDataSource.getRepository(Case_1.Case);
    const detectiveRepo = database_1.AppDataSource.getRepository(Detective_1.Detective);
    const assignmentRepo = database_1.AppDataSource.getRepository(CaseAssignment_1.CaseAssignment);
    // 배정되지 않은 사건 조회
 ...
 * 수동 배정
 */
async function manualAssignDetective(caseId, detectiveId, notes) {
    const caseRepo = database_1.AppDataSource.getRepository(Case_1.Case);
    const detectiveRepo = database_1.AppDataSource.getRepository(Detective_1.Detective);
    const assignmentRepo = database_1.AppDataSource.getRepository(CaseAssignment_1.CaseAssignment);
    const caseData = await caseRepo.findOne({ where: { id: caseId } });
    ...
    // 기존 배정 취소
    await assignmentRepo.update({ caseId, status: "assigned" }, { status: "reassigned" });
    // 카테고리 분류
    const { category } = await classifyCase({
        title: caseData.title,
        description: caseData.description || "",
...
async function rejectAssignment(assignmentId, reason) {
    ...
    if (assignment.detective) {
        assignment.detective.currentCaseCount = Math.max(0, assignment.detective.currentCaseCount - 1);
