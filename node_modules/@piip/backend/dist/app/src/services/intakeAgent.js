...
exports.startIntakeSession = startIntakeSession;
exports.selectTemplate = selectTemplate;
exports.processClientMessage = processClientMessage;
exports.getActiveTemplates = getActiveTemplates;
exports.getSessionWithHistory = getSessionWithHistory;
...
async function startIntakeSession() {
    const repo = database_1.AppDataSource.getRepository(IntakeSession_1.IntakeSession);
 ...
 * 의뢰 유형 선택 및 템플릿 적용
 */
async function selectTemplate(sessionId, templateId) {
    const sessionRepo = database_1.AppDataSource.getRepository(IntakeSession_1.IntakeSession);
    const templateRepo = database_1.AppDataSource.getRepository(RequestTemplate_1.RequestTemplate);
    ...
    const initialMessage = firstStep?.message ||
 ...
 * 클라이언트 메시지 처리 (핵심 로직)
 */
async function processClientMessage(sessionId, message) {
    const sessionRepo = database_1.AppDataSource.getRepository(IntakeSession_1.IntakeSession);
    const templateRepo = database_1.AppDataSource.getRepository(RequestTemplate_1.RequestTemplate);
    ...
    // 완료 여부 확인
    const missingFields = getMissingRequiredFields(template, session.collectedData);
    let agentMessage = "";
    ...
    if (missingFields.length === 0) {
        ...
        try {
            const { proposeConsultationAfterIntake } = await Promise.resolve().then(() => __importStar(require("./consultationGating")));
            const { classifyCase } = await Promise.resolve().then(() => __importStar(require("./caseAssignment")));
            // AI 신뢰도 추출 (classifyCase는 0-100 scale)
            const classifyResult = await classifyCase({
                title: session.collectedData.title || template.name,
                description: JSON.stringify(session.collectedData),
            });
    ...
    await sessionRepo.save(session);
    // AI 응답 기록
    await saveResponse(sessionId, "agent", agentMessage, session.currentStep, extractedData);
    return { agentMessage, isComplete, extractedData };
}
/**
 * AI로 사용자 메시지에서 정보 추출
 */
async function extractInformation(message, template, session) {
    const fields = template.fields.filter((f) => !(f.key in session.collectedData));
    if (fields.length === 0)
        return {};
...
위 메시지에서 추출 가능한 정보를 JSON으로 반환하세요.`;
    try {
        ...
        if (jsonMatch) {
            ...
            // 타입 변환 및 검증
            return validateAndTransform(extracted, template.fields);
        }
    }
    catch (err) {
 ...
 * 필드 값 검증 및 타입 변환
 */
function validateAndTransform(data, fields) {
    const result = {};
 ...
 * 누락된 필수 필드 확인
 */
function getMissingRequiredFields(template, collectedData) {
    return template.fields.filter((f) => f.required && !collectedData[f.key]);
}
/**
 * 다음 질문 생성
 */
async function generateNextQuestion(missingFields, template, session) {
    const nextField = missingFields[0];
    // 템플릿 기반 질문
    const conversationFlow = template.conversationFlow || [];
    const currentStep = conversationFlow.find((s) => s.expectedFields.includes(nextField.key));
 ...
 * 완료 메시지 생성
 */
async function generateCompletionMessage(session, template) {
    const systemPrompt = `당신은 친절한 탐정사무소 상담원입니다.
 ...
 * 세션으로부터 사건 자동 생성
 */
async function createCaseFromSession(session, template) {
    const caseRepo = database_1.AppDataSource.getRepository(Case_1.Case);
    const title = session.collectedData.title ||
        `${template.name} - ${session.clientName || "의뢰인"}`;
    const description = `[자동 접수]\n\n${template.description}\n\n수집된 정보:\n${Object.entries(session.collectedData)
        .map(([key, value]) => {
    ...
    // 자동 배정 활성화된 경우 배정 시도
    if ((process.env.AUTO_ASSIGNMENT_ENABLED || "true").toLowerCase() === "true") {
        try {
            const { autoAssignDetective } = await Promise.resolve().then(() => __importStar(require("./caseAssignment")));
            await autoAssignDetective(savedCase.id).catch((err) => {
 ...
 * 응답 저장
 */
async function saveResponse(sessionId, sender, message, stepNumber, extractedData) {
    const repo = database_1.AppDataSource.getRepository(IntakeResponse_1.IntakeResponse);
 ...
 * 세션 조회 (대화 기록 포함)
 */
async function getSessionWithHistory(sessionId) {
    const sessionRepo = database_1.AppDataSource.getRepository(IntakeSession_1.IntakeSession);
    const responseRepo = database_1.AppDataSource.getRepository(IntakeResponse_1.IntakeResponse);
    const session = await sessionRepo.findOne({
        where: { id: sessionId },
        relations: ["template"],
    });
