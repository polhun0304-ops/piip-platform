...
if (PROVIDER === "azure-openai") {
  ...
  if (
    ...
    try {
      ...
      console.log("IntakeAgent: Azure OpenAI client initialized");
    } catch (err) {
      console.error("IntakeAgent: Failed to load Azure OpenAI SDK:", err);
      azureClient = null;
    }
  } else {
    console.warn(
      "IntakeAgent: Azure OpenAI credentials not properly configured"
    );
  }
  ...
  if (apiKey && apiKey !== "your-api-key-here") {
    try {
      ...
      console.log("IntakeAgent: OpenAI client initialized");
    } catch (err) {
      console.error("IntakeAgent: Failed to load OpenAI SDK:", err);
      openaiClient = null;
    }
  } else {
    console.warn("IntakeAgent: OpenAI API key not properly configured");
  }
}
