  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const detectiveUtilization = await detectiveRepo
        ...
        .select(
          "AVG(detective.currentCaseCount * 100.0 / detective.maxConcurrentCases)",
          "avgUtilization"
        )
      ...
      const avgAssignmentTime = await assignmentRepo
        .createQueryBuilder("assignment")
        .select(
          "AVG(JULIANDAY(assignment.assignedAt) - JULIANDAY(assignment.createdAt))",
          "avgDays"
        )
      ...
      res.json({
        ...
        detectives: {
          ...
          avgUtilization: parseFloat(
            detectiveUtilization?.avgUtilization || "0"
          ),
    ...
    try {
      ...
      res.json({
        detective: {
          ...
          utilization: (
            (detective.currentCaseCount / detective.maxConcurrentCases) *
            100
          ).toFixed(1),
        },
        assignments: {
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const assignmentTrends = await assignmentRepo
        ...
        .addSelect(
          "SUM(CASE WHEN assignment.status = 'accepted' THEN 1 ELSE 0 END)",
          "accepted"
        )
      ...
      res.json({
        ...
        assignmentTrends: assignmentTrends.map((t) => ({
          ...
          acceptanceRate: (
            (parseInt(t.accepted) / parseInt(t.total)) *
            100
          ).toFixed(1),
        })),
      });
