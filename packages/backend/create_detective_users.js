...
(async () => {
  try {
    ...
    const possible = [
      ...
      path.join(__dirname, '..', 'packages', 'backend', 'dist', 'app', 'src', 'config', 'database'),
      path.join(__dirname, '..', 'packages', 'backend', 'dist', 'backend', 'src', 'config', 'database'),
    ];
    let dbModule;
    ...
    if (!dbModule) throw new Error('Cannot find compiled database module; looked at: ' + possible.join(', '));
    const AppDataSource = dbModule.AppDataSource;
    await AppDataSource.initialize();
    console.log('AppDataSource initialized');

    const DEFAULT_PW = process.env.DETECTIVE_DEFAULT_PASSWORD || 'detective123!';

    const detectives = await AppDataSource.query('SELECT id, email, name, phone FROM detective');
    console.log(`Found ${detectives.length} detectives`);
    let created = 0;
    for (const d of detectives) {
      const existing = await AppDataSource.query('SELECT id FROM user WHERE detectiveId = ? OR email = ? LIMIT 1', [d.id, d.email]);
      ...
      await AppDataSource.query(
        ...
        [id, d.email, hashed, d.name || d.email, d.phone || null, 'detective', 1, d.id]
      );
