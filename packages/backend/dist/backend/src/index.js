...
const e2ee_1 = __importDefault(require("./routes/e2ee"));
...
app.use("/api/e2ee", e2ee_1.default);
...
io.use((socket, next) => {
    try {
        ...
        if (!authValue) {
            ...
            if ((process.env.ALLOW_ANON_SOCKET || "false").toLowerCase() === "true" ||
        ...
        logger_1.default.warn("Socket handshake authorization failed: %s", msg);
...
io.on("connection", (socket) => {
    ...
    socket.on("join", (caseId) => {
        try {
            ...
            const msg = err instanceof Error ? err.message : String(err);
            logger_1.default.error("join error: %s", msg);
        }
    });
    socket.on("leave", (caseId) => {
        try {
            const room = `case_${caseId}`;
            socket.leave(room);
            ...
            const msg = err instanceof Error ? err.message : String(err);
            logger_1.default.error("leave error: %s", msg);
        }
    });
