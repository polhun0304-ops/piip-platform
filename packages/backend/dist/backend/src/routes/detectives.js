...
router.get("/", async (req, res) => {
    try {
        const { status, specialty, region, city, minSuccessRate, minRating, page = 1, limit = 10, sortBy = "averageRating", order = "DESC", } = req.query;
        ...
        const sortByValidated = allowedSortBy.includes(sortBy)
            ? sortBy
            : "averageRating";
        const orderValidated = allowedOrder.includes(order)
            ? order
            : "DESC";
        const [detectives, total] = await queryBuilder
            .orderBy(`detective.${sortByValidated}`, orderValidated)
...
router.post("/", auth_1.verifyJWT, auth_1.requireAdmin, async (req, res) => {
    try {
        const { name, phone, email, licenseNumber, experienceYears, specialties, maxConcurrentCases, bio, } = req.body;
        if (!name) {
 ...
 */
router.put("/:id", auth_1.verifyJWT, auth_1.requireAdminOrSelf, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, phone, email, licenseNumber, experienceYears, status, specialties, maxConcurrentCases, bio, workingHours, } = req.body;
        const repo = database_1.AppDataSource.getRepository(Detective_1.Detective);
        const detective = await repo.findOne({ where: { id } });
        if (!detective) {
            return res.status(404).json({ error: "Detective not found" });
...
router.post("/:id/specialties", async (req, res) => {
    try {
        ...
        // 기존 전문분야 찾기
        const existingIndex = detective.specialties.findIndex((s) => s.category === category);
        const specialty = {
            category,
            level,
    ...
    try {
        ...
        res.json({
            ...
            workload: {
                ...
                utilizationRate: Math.round((detective.currentCaseCount / detective.maxConcurrentCases) * 100),
            },
