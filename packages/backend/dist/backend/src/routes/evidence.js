...
router.post("/", auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        if (req.user?.role === "client") {
            ...
            const isAssigned = targetCase.assignments?.some((a) => a.detectiveId === req.user?.detectiveId);
            if (!isAssigned) {
                return res.status(403).json({ error: "Unauthorized" });
            }
        }
...
router.post("/upload", auth_1.verifyJWT, upload.single("file"), async (req, res) => {
    try {
        ...
        if ((process.env.ANALYSIS_ENABLED || "true").toLowerCase() === "true") {
            ...
            (0, analysisRunner_1.enqueueForEvidence)(saved.id).catch((err) => {
                console.error("Error enqueuing evidence analysis:", err);
            });
            // if case exists, also trigger case-level aggregate analysis
            if (saved.caseId) {
                (0, analysisRunner_1.enqueueForCase)(saved.caseId).catch((err) => {
