...
test('debug secure chat and AI analysis tabs', async ({ page }) => {
  ...
  // login via API to get token and set localStorage, and ensure a case exists
      ...
      if (casesRes.ok()) {
        ...
        if (!Array.isArray(cases) || cases.length === 0) {
          ...
          if (createRes.ok()) {
            ...
            firstCaseId = created.id || created._id || null;
          } else {
  ...
  // Navigate directly to known case detail if available
  if (firstCaseId) {
